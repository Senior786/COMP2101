...
#i am using while loop and switch to check the command line input using environment variable.

while [ $# -gt 0 ]; do
  case "$1" in
    -v )
        verbose="yes" # this case will be executed once the command line input is some value
        ;;
    * )
        if [ -z "$interface" ]; then
            interface="$1" # this case will work if there us no value provided while executing file
        else
