...
function error-message {
	echo "$1" >&2 #i am using $1 to give dynamic error message.
}
...
function error-exit {
  ...
  error-message "$1"
  exit "${2:-1}"	#this will send dynamic message to std which will be added in commmand line 1.
...
function cleanup {
  rm -f /tmp/*info.$$
  exit 1 # this function will cleanup all the temporary files
}
trap cleanup SIGINT
...
while [[ $# -gt 0 ]]; do
  case $1 in
    -h|--help)
      displayhelp
      exit # this is for displaying help
      ;;
    -v|--version)
      echo "The version of $0 file name is version 1.0"
      exit #this is for displaying version
      ;;
    *)
...
# this section is to print system information
echo "====================================="
